...
RPI_IP = '192.168.179.248'  # Raspberry Pi IP address
RPI_PORT = 5000  # Port on Raspberry Pi
RPI_BASE_URL = f'http://{RPI_IP}:{RPI_PORT}'
...
class ValveController:
    ...
    def connect(self, host):
        try:
            response = requests.post(f'{RPI_BASE_URL}/connect')
            if response.ok:
                self.connected = True
                logging.info(f"Connected to Raspberry Pi at {RPI_BASE_URL}")
                return True
            else:
                logging.error(f"Failed to connect to Raspberry Pi: {response.text}")
                return False
    ...
    def disconnect(self):
        try:
            response = requests.post(f'{RPI_BASE_URL}/disconnect')
            if response.ok:
                self.connected = False
                self.valve_states = [False, False, False]
                ...
                logging.error(f"Failed to disconnect from Raspberry Pi: {response.text}")
                return False
        except requests.exceptions.RequestException as e:
    ...
    def set_valve(self, valve_index, state):
        if not 0 <= valve_index < len(self.valve_states):
            return False
        try:
            response = requests.post(f'{RPI_BASE_URL}/valve', 
                json={'valveIndex': valve_index, 'state': state})
            if response.ok:
                self.valve_states[valve_index] = state
                logging.info(f"Valve {valve_index} set to {state} on Raspberry Pi")
                return True
            else:
                logging.error(f"Failed to set valve on Raspberry Pi: {response.text}")
                return False
        except requests.exceptions.RequestException as e:
            logging.error(f"Error setting valve on Raspberry Pi: {e}")
            return False
...
if __name__ == '__main__':
    ...
    logging.info(f"Starting pipeline server on http://{local_ip}:5001")
